...
class Article < ApplicationRecord
  ...
  def self.filtered(params, _current_user)
    ...
    if params[:author_first_name].present?
      scope = scope.joins(:author).where(author: { first_name: "#{params[:author_first_name].strip}" })
    end
    if params[:author_last_name].present?
      scope = scope.joins(:author).where(author: { last_name: "#{params[:author_last_name].strip}" })
    end
    if params[:category_name].present?
      scope = scope.joins(:category).where(category: { name: "#{params[:category_name].strip}" })
    end
    scope = scope.joins(:tags).where(tags: { name: "#{params[:tag_name].strip}" }) if params[:tag_name].present?
